...
class VectorField : public Eigen::VectorXd
{
    public:
        ...
        inline vec3d vec_at(const int pos) const
        {
            int ptr = pos * 3;
            return vec3d((*this)[ptr], (*this)[ptr+1], (*this)[ptr+2]);
        }

        inline void set(const int pos, const vec3d & vec)
        {
            int ptr = pos * 3;
            (*this)[ptr + 0] = vec.x();
            (*this)[ptr + 1] = vec.y();
        ...
        //
        template<typename OtherDerived>
        VectorField(const Eigen::MatrixBase<OtherDerived>& other) : Eigen::VectorXd(other) {}
};
