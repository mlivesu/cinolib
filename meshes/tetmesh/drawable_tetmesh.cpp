...
void DrawableTetmesh<M,V,E,F,C>::updateGL_out()
{
    ...
    for(uint fid=0; fid<this->num_faces(); ++fid)
    {
        ...
        drawlist_out.tri_text.push_back((this->vert_data(vid0).uvw[0] - min)/delta);
        drawlist_out.tri_text.push_back((this->vert_data(vid1).uvw[0] - min)/delta);
        drawlist_out.tri_text.push_back((this->vert_data(vid2).uvw[0] - min)/delta);
    ...
    for(uint cid=0; cid<this->num_cells(); ++cid)
    {
        ...
        for(uint nbr : this->adj_c2c(cid))
        {
            if (!(this->cell_data(nbr).visible))
            {
                ...
                drawlist_in.tri_text.push_back((this->vert_data(vid0).uvw[0] - min)/delta);
                drawlist_in.tri_text.push_back((this->vert_data(vid1).uvw[0] - min)/delta);
                drawlist_in.tri_text.push_back((this->vert_data(vid2).uvw[0] - min)/delta);
